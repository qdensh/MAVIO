...
#include <ack_msckf_lam/TrackingInfo.h>
...
ImageProcessorAckLam::ImageProcessorAckLam(ros::NodeHandle& n) :
  nh(n),
  is_first_img(true),
...
ImageProcessorAckLam::~ImageProcessorAckLam() {
  destroyAllWindows();
  return;
}
 ...
 *
 */
bool ImageProcessorAckLam::loadParameters() {
  ...
  std::string delim = ",";
  time_file.open((output_path + "time_image_ack_msckf_lm" + "_" + file_time + ".csv"), std::ios::out);
  if(!time_file .is_open())
  {
      cerr << "time_image_ack_msckf_lm  is not open" << endl;
  }
  time_file  << "#Time(sec),";
  time_file  << "Dtime(sec),";
  time_file  << delim;
  time_file  << "image_process_time(ms),image_total_time(ms),image_avg_time(ms),";
  time_file  << std::endl;
 ...
 *
 */
bool ImageProcessorAckLam::createRosIO() {
  ...
  cam0_img_sub.registerCallback(&ImageProcessorAckLam::cam0Callback, this);
  imu_sub = nh.subscribe("imu", 50,
      &ImageProcessorAckLam::imuCallback, this);

  // save csv 1s
  csv_timer = nh.createTimer(ros::Duration(1),&ImageProcessorAckLam::csv_timer_callBack,this);
 ...
 *
 */
bool ImageProcessorAckLam::initialize() {
 ...
 *
 */
void ImageProcessorAckLam::imuCallback(
 ...
 *
 */
void ImageProcessorAckLam::createImagePyramids() {
  cv::Mat img;
 ...
 *
 */
void ImageProcessorAckLam::initializeFirstFrame() {
  // Size of each grid.
  ...
  for (auto& item : grid_new_features)
    std::sort(item.second.begin(), item.second.end(),
        &ImageProcessorAckLam::featureCompareByResponse);
 ...
 *
 */
void ImageProcessorAckLam::predictFeatureTracking(
 ...
 *
 */
void ImageProcessorAckLam::trackFeatures() {
  // Size of each grid.
  ...
  // Compute a rough relative rotation
  Matx33f cam0_R_p_c;
 ...
 *
 */
void ImageProcessorAckLam::addNewFeatures() {
  ...
  for (auto& item : new_feature_sieve) {
    if (item.size() > processor_config.grid_max_feature_num) {
      std::sort(item.begin(), item.end(),
          &ImageProcessorAckLam::keyPointCompareByResponse);
      item.erase(
 ...
 *
 */
void ImageProcessorAckLam::pruneGridFeatures() {
  for (auto& item : *curr_features_ptr) {
    ...
    std::sort(grid_features.begin(), grid_features.end(),
        &ImageProcessorAckLam::featureCompareByLifetime);
 ...
 *
 */
void ImageProcessorAckLam::undistortPoints(
 ...
 *
 */
vector<cv::Point2f> ImageProcessorAckLam::distortPoints(
    const vector<cv::Point2f>& pts_in,
    const cv::Vec4d& intrinsics,
    const string& distortion_model,
    const cv::Vec4d& distortion_coeffs) {
 ...
 *
 */
void ImageProcessorAckLam::integrateImuData(
    Matx33f& cam0_R_p_c) {
 ...
 *
 */
void ImageProcessorAckLam::rescalePoints(
 ...
 *
 */
void ImageProcessorAckLam::twoPointRansac(
 ...
 *
 */
void ImageProcessorAckLam::publish() {
 ...
 *
 */
void ImageProcessorAckLam::drawFeaturesMono() {
 ...
 *
 */
void ImageProcessorAckLam::drawFeaturesStereo() {
 ...
 *
 */
void ImageProcessorAckLam::updateFeatureLifetime() {
  for (int code = 0; code <
      processor_config.grid_row*processor_config.grid_col; ++code) {
 ...
 *
 */
void ImageProcessorAckLam::featureLifetimeStatistics() {
 ...
 *
 */
void ImageProcessorAckLam::csv_timer_callBack(const ros::TimerEvent& event){
  ...
  if(ros::Time::now().toSec() - csv_curr_time > 1){
    ...
    std::cout << "-------image time_file save done!!!---------" << std::endl;
 ...
 *
 */
void ImageProcessorAckLam::cam0Callback(
  ...
  if (is_first_img) {
    ...
    // save first time
    DfirstTime = cam0_img->header.stamp.toSec();
  } else {
    // save total Dtime
    Dtime = cam0_img->header.stamp.toSec() - DfirstTime;
  ...
  csvdata_time.time = cam0_img->header.stamp.toSec();
